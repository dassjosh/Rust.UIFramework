...
public readonly record struct UiBorderWidth(float Left, float Top, float Right, float Bottom)
{
    ...
    public static UiBorderWidth Parse(ReadOnlySpan<char> input, ReadOnlySpan<char> token = " ") => TryParse(input, out UiBorderWidth result, token) ? result : throw new FormatException($"Unable to parse '{input}' as {nameof(UiBorderWidth)}");
