...
public static class FieldAnimationExt
{
    extension<T>(in AnimationRef<IFieldAnimation<T>> field)
    {
        ...
        public AnimationRef<IFieldAnimation<T>> Lerp(T start, T end, UiLerp<T> lerp) => field.IsValid ? field.WithAnimator(LerpAnimator<T>.Create(field.Plugin, start, end, lerp)) : field;
    }

    extension(in AnimationRef<IFieldAnimation<string>> field)
    {
        public AnimationRef<IFieldAnimation<string>> Lerp<T>(T start, T end, string format = null, IFormatProvider formatProvider = null) where T : unmanaged, IFormattable
            => field.IsValid ? field.Lerp(start, end, UiLerp.GetDefault<T>(), format, formatProvider) : field;

        public AnimationRef<IFieldAnimation<string>> Lerp<T>(T start, T end, UiLerp<T> lerp, string format = null, IFormatProvider formatProvider = null) where T : unmanaged, IFormattable
